                    ...
                    @if($workloads->count() >0)
                        ...
                        <tr>
                            <td class="col-sm-1">
                                ...
                                <a href="{{route('employee.workload.detail',$workload->id)}}" data-toggle="tooltip" data-placement="right" title="" data-original-title="Chi tiết" href="javascript:" class="search_tag tooltip-test">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btn-import-submit").on('click', function(e) {
                ...
                var url_getdata = '{{route('
                admin.pi.import.data ')}}';
                ...
                $.ajax({
                    ...
                    success: function(datas) {
                        if ($.isEmptyObject(datas.error)) {
                            ...
                            $.each(datas[0], function(index, value) {
                                ...
                                if (index == 0) {
                                    $.each(datas[0][0], function(index1, value1) {
                                        $('.heading-table').append($('<th>', {
                                            'text': value1,
                                            'class': 'heading-table-import-tr text-nowrap'
                                        }));
                            ...
                            $.each(datas[0], function(index2, value2) {
                                $('.row-table').append($('<tr>', {
                                    'class': 'row-table-import-tr text-nowrap'
                                }));
                                $.each(datas[0][index2], function(index3, value3) {
                                    $('.row-table').children('tr:last').append($('<td>', {
                                        'text': value3
                                    }));
                                });
                            });
                            ...
                            $.each(datas[1], function(index, value) {
                                if (index == 0) {
                                    $.each(datas[1][0], function(index1, value1) {
                                        $('.heading-table-2').append($('<th>', {
                                            'text': value1,
                                            'class': 'heading-table-import-tr-1 text-nowrap'
                                        }));
                            ...
                            $.each(datas[1], function(index2, value2) {
                                $('.row-table-2').append($('<tr>', {
                                    'class': 'row-table-import-tr-1 text-nowrap'
                                }));
                                $.each(datas[1][index2], function(index3, value3) {
                                    $('.row-table-2').children('tr:last').append($('<td>', {
                                        'text': value3
                                    }));
                                });
                            });
                    ...
                    error: function(er) {
                        ...
                        $(".print-error-msg").find("ul").html('');
                        $(".print-error-msg").css('display', 'block');
                        $(".print-error-msg").find("ul").append('<li>' + 'File tải lên không đúng cấu trúc .Vui lòng xem lại file mẫu <small> <a href="{{route('
                            admin.pi.template.download ')}}"> (tải file mẫu)</a></small>' + '</li>');
                    },
                });
